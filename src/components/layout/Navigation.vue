  ...
  <nav class="w-full h-[120px] items-center flex relative z-50">
    <div class="container mx-auto px-6 py-4 flex justify-between items-center">
      ...
      <div class="hidden sm:flex items-center gap-2">
        <RouterLink 
          to="/" 
          class="px-4 py-2 rounded-md font-inter text-sm transition-all duration-200 relative overflow-hidden"
        ...
        >
          ...
          <span class="absolute bottom-0 left-0 h-[2px] w-0 bg-gradient-bitcoin hover:w-full transition-all duration-300"
                :class="currentPath === '/' ? 'w-full' : 'w-0'"></span>
        </RouterLink>
        <RouterLink 
          to="/generate" 
          class="px-4 py-2 rounded-md font-inter text-sm transition-all duration-200 relative overflow-hidden"
        ...
        >
          <span>Generate</span>
          <span class="absolute bottom-0 left-0 h-[2px] w-0 bg-gradient-bitcoin hover:w-full transition-all duration-300"
                :class="currentPath === '/generate' ? 'w-full' : 'w-0'"></span>
        </RouterLink>
        
        <a 
          href="https://github.com/yourusername/bitcoin-widgets" 
          target="_blank"
          class="ml-2 p-2 text-slate-700 hover:text-slate-900 transition-colors duration-200"
      ...
      >
        <div class="w-6 h-0.5 bg-black mb-1.5 transition-all duration-300"
             :class="isMenuOpen ? 'rotate-45 translate-y-2 bg-bitcoin-orange' : ''"></div>
        <div class="w-6 h-0.5 bg-black mb-1.5 transition-all duration-300"
    ...
    >
      <div class="flex flex-col items-center justify-center h-[calc(100vh-120px)]">
        <RouterLink 
          to="/" 
          class="px-3 py-4 text-xl font-medium transition-all duration-200 relative"
        ...
        >
          ...
          <span class="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] w-0 bg-gradient-bitcoin transition-all duration-300"
                :class="currentPath === '/' ? 'w-1/2' : 'w-0'"></span>
        </RouterLink>
        <RouterLink 
          to="/generate" 
          class="px-3 py-4 text-xl font-medium transition-all duration-200 relative"
        ...
        >
          ...
          <span class="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] w-0 bg-gradient-bitcoin transition-all duration-300"
                :class="currentPath === '/generate' ? 'w-1/2' : 'w-0'"></span>
        </RouterLink>
